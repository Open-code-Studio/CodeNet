...
namespace
{
    ...
    // Get the CodeNet search options that should be used
    ...
    bool try_get_dotnet_search_options(fxr_resolver::search_location& out_search_location, pal::string_t& out_app_relative_dotnet)
    {
        ...
        static_assert(sizeof(EMBED_DOTNET_SEARCH_FULL_UTF8) / sizeof(EMBED_DOTNET_SEARCH_FULL_UTF8[0]) < EMBED_SIZE, "Placeholder value for CodeNet search options longer than expected");

        // Contains the EMBED_DOTNET_SEARCH_FULL_UTF8 value at compile time or app-relative CodeNet path written by the SDK (dotnet publish).
        static char embed[EMBED_SIZE] = EMBED_DOTNET_SEARCH_FULL_UTF8;

        out_search_location = (fxr_resolver::search_location)embed[0];
        assert(embed[1] == 0); // NUL separates the search location and embedded CodeNet root value
        if ((out_search_location & fxr_resolver::search_location_app_relative) == 0)
            return true;

        // Get the embedded app-relative CodeNet path
